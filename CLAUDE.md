...
**Probes.** `/healthz` + `/v1/healthz` are liveness (static 200). `/readyz` + `/v1/readyz` are readiness: they run checks registered via `RegisterReadinessCheck(name, func(ctx) error)`, concurrently, bounded by `HTTP_READINESS_TIMEOUT`, and answer 503 if any fails. All four paths are skipped by the request logger, the rate limiter and gzip — one set, `probePaths` in `http.go`, behind `onProbePath`. Readiness bodies name the failing check but include its error text only when `HTTP_DEBUG` is set — the cause always goes to the log at warn instead (see `readiness.go`, which logs transitions plus one repeat per minute rather than per probe).

`probePaths` holds **only the four routes this package registers**, and it stays that way. A consumer's own `/health` or `/v1/health` — note the missing `z`; they are different paths and this package registers neither — is not in it, and adding them (or registering them as routes) would have the package stop logging traffic it does not own. Such a consumer extends the skip set with `HTTP_LOG_EXEMPT_PATHS`, resolved in `NewHTTP` and passed to `requestLogger`. That list feeds **the access log only**, not the rate limiter and not gzip: silently removing a route's rate limit through a variable named for logging is a security-relevant surprise, and gzip's 1 KiB minimum already skips a two-word probe body on its own. `skipProbesAndExempt` is the shared skipper — probe paths plus one configured list — used by the logger and by gzip, so their matching cannot drift.
...
- `HTTP_RATE_LIMIT` (req/s per client IP, default `0` = off) and `HTTP_RATE_LIMIT_BURST` (defaults to one second's worth, rounded up, never zero). Off by default: per-process limiter, in-memory store keyed on `c.RealIP()`, so it multiplies by replica count and inherits `HTTP_REAL_IP_SOURCE`. Probe paths are never limited — a 429 on `/readyz` or `/healthz` would turn load into an outage — but that means the built-in four only, and `HTTP_LOG_EXEMPT_PATHS` does not extend it. A burst with no rate is a startup error.
...
- `HTTP_LOG_EXEMPT_PATHS` (default empty) — comma-separated route patterns the **access log** skips in addition to the four probe paths, for a consumer whose own probes are `/health`, `/v1/health`, `/status`, `/ping` rather than `/healthz`. Additive: the built-in four are not removable, and the default is empty so a consumer that sets nothing logs what it always did. Same validation as the timeout list (both go through `exemptPaths`, which trims, drops blanks and refuses an entry without a leading `/`; the error names which of the two lists it came from) and the same exact match on the route pattern, so `/v1/health` does not exempt `/v1/healthcheck`. Log only — see the probes paragraph above.
